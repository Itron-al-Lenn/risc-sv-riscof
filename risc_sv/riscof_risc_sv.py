...
class risc_sv(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","risc_sv")
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # Build simulation using Verilator
      execute = 'sh sim/sim_build.sh'
    ...
    def runTests(self, testList):
      ...
      for testname in testList:
          logger.debug(f'Running Test: {testname} on DUT')
          ...
          execute = f'cp -f {test_dir}/{elf} ./sim/{elf}'
          logger.debug('DUT executing ' + execute)
          utils.shellCommand(execute).run()

          # convert to HEX memory initialization file
          execute = 'sh sim/conv_elf.sh'
          logger.debug('DUT executing ' + execute)
          utils.shellCommand(execute).run()

          # execute Verilator simulation
          execute = 'sh sim/sim_run.sh'
          logger.debug('DUT executing ' + execute)
          utils.shellCommand(execute).run()
          ...
          execute = f'cp -f ./sim/DUT-risc-sv.signature {test_dir}/.'
          logger.debug('DUT executing ' + execute)
          utils.shellCommand(execute).run()
